...
void getChoice()
{
Bus::source=new char(Bus::source+1);
Bus::destination=new char(Bus::destination+1);
}



int Bus::getBus(const char *s,const char *d)
{
Book* book = xlCreateBook();
int row;
const char *temp;
...
std::cout<<"1. Want To Initilize Route Table.(Y/N)"<<endl;
